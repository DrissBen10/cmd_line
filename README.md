...
``` bash
curl -s https://raw.githubusercontent.com/DrissBen10/cmd_line/refs/heads/main/run.sh | sudo bash -s odoo-one 10017 20017
```
and/or run the following to set up another Odoo instance @ `localhost:11017` (default master password: `minhng.info`):

``` bash
curl -s https://raw.githubusercontent.com/DrissBen10/cmd_line/refs/heads/main/run.sh | sudo bash -s odoo-two 11017 21017
```
